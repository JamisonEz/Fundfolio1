		...
		function GetTransactionDetails(){
			...
			if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
				...
				*/
                                ...
                                $folioid = urldecode($httpParsedResponseAr['L_NUMBER0']);
                                $txnid = urldecode($httpParsedResponseAr['TRANSACTIONID']);
                                
                                $data['txn_id'] = 'paypal_'.$txnid;
                                $data['payment_amount'] = $amount;
                                $data['payment_status'] = 'success';
                                $data['item_number'] = $folioid;
                                $data['userid'] = $user_info['user_id'];
                                
                                $db->updatePayments($data);
                                $db->updateFolioMatrix($folioid, $amount, $user_info['user_id']);
                                $db->addCommunityPoints($user_info['user_id'], $amount);
                                ...
                                {
                                    $dir = 'Fundfolio1-master';
                                }
                                
                                header("Location: /$dir/usercampaign.php?folio_id=$folioid");
                                die();
