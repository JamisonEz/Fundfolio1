    ...
    // invalid emailaddress
		...
		if($db -> CheckLogin()){
                ...
                if(isset($_GET['code'])){
                        ...
                        $_SESSION['g_token'] = $google_client->getAccessToken();
                        header('Location: ' . filter_var($redirect_url, FILTER_SANITIZE_URL));
                }

                if (isset($_SESSION['g_token'])) {
                        $google_client->setAccessToken($_SESSION['g_token']);
                }
                
                if ($google_client->getAccessToken() && isset($_SESSION['g_token'])) {
                    
                    //Get user profile data from google
                    $google_user_data = $google_oauth_v2->userinfo->get();

                    //Insert or update user data to the database
                    $g_user_data = array(
                        'oauth_provider'=> 'google',
                        'oauth_uid'     => $google_user_data['id'],
                        'first_name'    => $google_user_data['given_name'],
                        'last_name'     => $google_user_data['family_name'],
                        'email'         => $google_user_data['email'],
                        'gender'        => $google_user_data['gender'],
                        'locale'        => $google_user_data['locale'],
                        'picture'       => $google_user_data['picture'],
                        'link'          => $google_user_data['link']
                    );
                    
                    $alread_added_flag = $db->checkEmailForGoogleLogin($g_user_data['email']);
                    if($alread_added_flag)
                    {
                        //login the user
                        $db->login($g_user_data['email'], '', true);
                    }
                    else
                    {
                        $temp_num = '';
                        $image = '';
                        if($g_user_data['picture']!="")
                        {
                            $temp_num = round(microtime(true));
                            copy('http://www.google.co.in/intl/en_com/images/srpr/logo1w.png', '/profile_uploads/img_'.$temp_num.'jpg');
                        }
                        if(file_exists('/profile_uploads/img_'.$temp_num.'jpg'))
                        {
                            $image = '/profile_uploads/img_'.$temp_num.'jpg';
                        }
                        $res = $db->userRegister($g_user_data['email'], '',$g_user_data['first_name'], '', $image, $g_user_data['oauth_uid']);
                        if( $res == 1){
                            //echo "Sucsess";
                            header("Location: ".dirname('__DIR__')."/homescreen.php");
                            die();
    ...
    <div>
        ...
        <div id="main_content" style="position: relative; display: inline-block; overflow: hidden; width: 100%;">
                ...
                <div id="login_screen" class="row" style="width: 30%; position: absolute; left: 70%; height: auto; visibility: hidden; background-color: white; margin-bottom: 0px">
                    ...
                    <div class="row" style="margin-top: 10px; margin-bottom: 0px">
                        <form class="col s12" method="post" name = "register" id = "register" action = "" enctype="multipart/form-data" style="padding: 0; margin-top: 10px">						
                            ...
                            <?php $auth_url = $google_client->createAuthUrl(); ?>
                            <a href="<?php echo filter_var($auth_url, FILTER_SANITIZE_URL)?>"><input type="image" src="images/google.png" alt="Submit" width="100%" style="margin-top: -6px; margin-bottom: -6px"></a>
            ...
            <div id="tag_img" class="row" style="margin-top: 20%; margin-left: 90px;margin-bottom: 0;">
                ...
                <div class="col s12">
                    <a href="http://launchafolio.com" class="waves-effect waves-ripple btn-large white" style="color: black">GET STARTED</a>
                </div>
            </div>
        </div>
